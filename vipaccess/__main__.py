...
def provision(p, args):
    ...
    try:
        ...
        if e.args == ("Unsupported token model", "4E0D"):
            p.error(
                "Unsupported token model {!r}.\n" "     See list at {}".format(args.token_model, TOKEN_MODEL_REFERENCE_PAGE)
            )
        p.error("Provisioning server error {}: {}".format(e.args[1], e.args[0]))
    ...
    if args.print:
        ...
        if otp_token["period"] is not None and otp_token["counter"] is None:
            ...
            c = otp_token["counter"]
            print(
                "    oathtool    {}-c{} -b --hotp {}  # output next code (need to increment counter each time!)"
                "".format(d, c, otp_secret_b32)
            )
...
def show(p, args):
    if args.secret:
        ...
        if args.verbose:
            if "id" in d:
                print("Token ID: %s" % d["id"], file=sys.stderr)
            if "expiry" in d:
                print("Token expiration: %s" % d["expiry"], file=sys.stderr)
            sys.stderr.write("\n")
...
def main():
    ...
    pprov.add_argument(
        "-i", "--issuer", default="VIP Access", action="store", help="Specify the issuer name to use (default: %(default)s)"
    )
    pprov.add_argument(
        "-t",
        "--token-model",
        default="SYMC",
        type=check_token_model,
    ...
    m.add_argument(
        "-s", "--secret", action=UnsetDotfileAndStore, nargs=1, help="Specify the token secret to test (base32 encoded)"
    )
    pcheck.add_argument("-I", "--identity", help="Specify the ID of the token to test (normally starts with VS or SYMC)")
    ...
    puri.add_argument(
        "-i", "--issuer", default="Symantec", action="store", help="Specify the issuer name to use (default: Symantec)"
    )
    puri.add_argument("-I", "--identity", action="store", help="Specify the ID of the token to use (required with --secret))")
